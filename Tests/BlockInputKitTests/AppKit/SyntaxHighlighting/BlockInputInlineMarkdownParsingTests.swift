...
final class BlockInputInlineMarkdownParsingTests: XCTestCase {
    ...
    func testFourAsterisksAroundWordAreStyled() {
        let ranges = BlockInputInlineMarkdownParsing.inlineMarkdownRanges(in: "****bold****")

        XCTAssertFalse(ranges.isEmpty, "****bold**** should produce at least one styled range")
    }

    func testFiveAsterisksAroundWordAreStyled() {
        let ranges = BlockInputInlineMarkdownParsing.inlineMarkdownRanges(in: "*****bold*****")

        XCTAssertFalse(ranges.isEmpty, "*****bold***** should produce at least one styled range")
    }

    func testFourAsterisksContentIsCorrect() {
        // ****bold**** → *** opens bold+italic, leaving *bold* italic content with one extra * on each side
        // After the *** pass consumes the outer 3, the * pass sees *bold* and applies italic
        let text = "****bold****"
        let ranges = BlockInputInlineMarkdownParsing.inlineMarkdownRanges(in: text)
        let styles = ranges.map(\.style).sorted { $0.hashValue < $1.hashValue }

        XCTAssertTrue(styles.contains(.bold), "****bold**** should be bold")
        XCTAssertTrue(styles.contains(.italic), "****bold**** should be italic")
    }
