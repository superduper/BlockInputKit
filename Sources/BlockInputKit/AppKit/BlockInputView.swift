...
public final class BlockInputView: NSView {
    ...
    /// Shape of the text insertion point inside block text views. Changing this invalidates visible text views.
    public var insertionPointStyle = BlockInputInsertionPointStyle.bar {
        didSet { applyInsertionPointStyleChange(from: oldValue) }
    }
    ...
    var isPreferredHeightCallbackScheduled = false, imageLoader: any BlockInputImageLoading = BlockInputDefaultImageLoader()
    var imageDiskCache: (any BlockInputImageDiskCaching)?, blockContentRenderers = BlockInputBlockContentRendererRegistry()
    var imageBaseURL: URL?, fileBaseURL: URL?
    var renderedContentZoomProvider: (@MainActor (BlockInputRenderedContentZoomContext) -> NSView?)?
    var resolvedContentDimensionsByBlockID: [BlockInputBlockID: BlockInputImageDimensions] = [:]
    var interactiveBlockContentProvider: (@MainActor (BlockInputInteractiveBlockContent.Context) -> (any BlockInputInteractiveBlockContent.View)?)?
    var blockContentAIBackend: (any BlockInputInteractiveBlockContent.AIBackend)?
    ...
    let pinchZoomController = BlockInputPinchZoomController(), diagramFullscreenWindow = BlockInputContentFullscreenWindow()
    ...
    var keyDownHandler: ((NSEvent) -> Bool)?, isPerformingDefaultKeyboardShortcut = false
    var isIgnoringShortcutDispatch = false, ignoredKeyboardShortcutEventIDs: Set<ObjectIdentifier> = []
    var completionProvider: (any BlockInputCompletionProvider)?
    var fileDropHandler: BlockInputFileDropHandler?, fileDropTasks: [UUID: Task<Void, Never>] = [:]
    var pasteContentHandlers: [any BlockInputPasteContentHandler] = [], pasteContentTasks: [UUID: Task<Void, Never>] = [:]
    ...
    /// Host-registered completion token triggers (multi-char openers), searched before the single-char `@`/`/` path.
    var completionTokenTriggers: [BlockInputCompletionTokenTrigger] = []
    /// Host-registered async source rewriters run off the render hot path.
    var inlineMarkupRewriters: [any BlockInputInlineMarkupRewriter] = []
    /// `(identifier, blockID)` pairs with an outstanding rewrite, so each rewriter runs at most once per block at a time.
    var inlineMarkupRewritesInFlight: Set<BlockInputInlineMarkupRewriteKey> = []
    var linkHoverEditAffordance = true, completionPopupConfiguration = BlockInputCompletionPopupConfiguration()
    var onDocumentMutation: ((BlockInputDocumentChange) -> Void)?, onDocumentChange: ((BlockInputDocument) -> Void)?
    var documentChangeSnapshotDelay: TimeInterval = 0.25
    var onSelectionChange: ((BlockInputSelection?) -> Void)?, onFocusChange: ((Bool) -> Void)?
    ...
    var selectedHorizontalRuleIndex: Int?, preferredNavigationX: CGFloat?
    // Invalidates deferred selection restoration when a later reload should not restore focus.
    // `isBecomingFirstResponder` avoids re-entering NSWindow first-responder assignment while promoting.
    ...
    // `linkModalContext` is the captured source context; save/remove actions verify it before mutating block text.
    var linkModalView: BlockInputLinkModalView?, linkModalContext: BlockInputLinkContext?
    var imageModalView: BlockInputImageModalView?, imageModalContext: BlockInputImageContext?
    ...
    // Editor-owned hover Edit affordance for inline links; see BlockInputView+LinkHover.swift.
    var linkHoverPopover: BlockInputLinkHoverPopover?, linkHoverTarget: BlockInputLinkHoverTarget?
    var linkHoverDismissWorkItem: DispatchWorkItem?
    /// Whether each rendered link/wikilink gets a presentation-only inline "open" icon at its trailing edge.
    var showsInlineLinkOpenButton = true
    var completionSession: BlockInputCompletionSession?, completionRequestTask: Task<Void, Never>?
    ...
    // Live state for the link modal's "Target" fuzzy note finder. Reuses the same completion provider/ranker and the
    // same popup view/state as the inline `[[` finder; see BlockInputView+ModalCompletion.swift.
    var modalCompletionState: BlockInputModalCompletionState?
    var modalCompletionRequestTask: Task<Void, Never>?
    var modalCompletionPopupView: BlockInputCompletionPopupView?
    let modalCompletionPopupEventCaptureView = BlockInputCompletionEventCaptureView()
    nonisolated(unsafe) var modalCompletionPopupMouseDownMonitor: Any?
    var modalCompletionPopupConsumesNextMouseUp = false
    // Host-supplied selection-anchored overlay (e.g. a floating toolbar). Menu-agnostic: core only positions the view.
    var selectionOverlayProvider: BlockInputSelectionOverlayProvider?, selectionOverlayView: NSView?
    nonisolated(unsafe) var selectionOverlayMouseDownMonitor: Any?
    ...
    deinit {
        ...
        // View deinit runs on the main thread; break the controller↔recognizer cycle and stop any glide timer.
        MainActor.assumeIsolated { pinchZoomController.detach() }
