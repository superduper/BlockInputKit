...
extension BlockInputView {
    ...
    private func scrollActiveTextSelectionToVisibleNow() {
        // Scroll the document scroll view so the caret is visible.  This is always
        // necessary: the text view's scrollRangeToVisible only reaches its enclosing
        // block-item scroll view, which never clips because the item grows to fit
        // content. The outer document scroll view must be scrolled explicitly.
        guard window != nil,
              let caret = activeTextSelectionCaret else {
            return
        }
        collectionView.layoutSubtreeIfNeeded()
        // Fast path: item is mounted — use the exact caret rect for pixel-accurate scrolling.
        if let item = visibleItemForActiveTextSelection(blockID: caret.blockID) {
        ...
        // Fallback: item is off-screen (e.g. inserted at end of a long document).
        // Use layout attributes to find the block's frame and scroll to show it.
        guard let index = document.index(of: caret.blockID) else {
            return
        }
        let indexPath = IndexPath(item: index, section: 0)
        if let frame = collectionView.collectionViewLayout?.layoutAttributesForItem(at: indexPath)?.frame,
           frame != .zero {
            scrollDocumentRectToVisibleIfNeeded(frame)
        }
    }
