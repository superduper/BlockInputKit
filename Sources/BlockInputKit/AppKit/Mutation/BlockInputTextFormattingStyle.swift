...
struct TextFormattingStyle {
    ...
    /// The inline markdown style this formatter targets, or `nil` for inline code.
    ///
    /// Inline code is not represented by ``BlockInputInlineMarkdownStyle`` because backtick spans
    /// are parsed separately (see ``BlockInputCodeParsing/inlineCodeRanges(in:)``) and excluded from
    /// markdown parsing. When `markdownStyle` is `nil`, ``isInlineCode`` is `true`.
    let markdownStyle: BlockInputInlineMarkdownStyle?

    /// Whether this formatter toggles inline code (single backticks) rather than a markdown style.
    var isInlineCode: Bool { markdownStyle == nil }
    ...
    private func enclosingFormattedRange(in text: String, selectedRange: NSRange) -> FormattingRange? {
        if isInlineCode {
            return enclosingInlineCodeRange(in: text, selectedRange: selectedRange)
        }
        let nsText = text as NSString
    ...
    /// Detects an existing inline-code span enclosing the selection using ``BlockInputCodeParsing``.
    ///
    /// Inline code is parsed separately from inline markdown, so toggle-off detection must consult
    /// the backtick spans directly. Each span maps its `delimiterRanges[0]`/`[1]` to the opening and
    /// closing delimiter ranges and its `contentRange` to the formatted content.
    private func enclosingInlineCodeRange(in text: String, selectedRange: NSRange) -> FormattingRange? {
        BlockInputCodeParsing.inlineCodeRanges(in: text)
            .compactMap { range -> FormattingRange? in
                ...
                return FormattingRange(
                    openingRange: range.delimiterRanges[0],
                    closingRange: range.delimiterRanges[1],
                    contentRange: range.contentRange,
                    selectedContentRange: selectedContentRange
                )
            }
    ...
    private func removalRanges(
        ...
        case .underline, .strikethrough, .link, .customMarkup, .rawSlashCommand, .none:
            return (openingRange, closingRange)
        }
    }
}
