...
extension BlockInputView: BlockInputBlockItemDelegate {
    func blockItemDidBeginEditing(_ item: BlockInputBlockItem, blockID: BlockInputBlockID) {
        ...
        // User clicked into a block (find never focuses a block): drop the dim, keep highlights.
        dismissFindDimForUserInteraction()
    ...
    func blockItem(
        ...
        if applyTypingShortcutAndReconfigureIfNeeded(
            item: item,
            blockID: blockID,
        ...
        if splitParagraphAtBlankLinesIfNeeded(
            blockID: blockID,
            beforeBlock: beforeBlock,
        ...
        // Edited text may add/remove matches: refresh highlights (no caret move) so stale ones drop.
        refreshFindMatchesAfterDocumentEdit()
    }

    /// Applies a Markdown typing shortcut (e.g. `# `, `- `) if one matches, reconfiguring the item when it still
    /// owns the edited block. Returns true when a shortcut consumed the change (caller should stop processing it).
    private func applyTypingShortcutAndReconfigureIfNeeded(
        item: BlockInputBlockItem,
        blockID: BlockInputBlockID,
        resolvedChange: (text: String, proposedOffset: Int),
        selectionBefore: BlockInputSelection?
    ) -> Bool {
        guard let shortcutSelection = applyTypingShortcutIfNeeded(
            blockID: blockID,
    ...
    func blockItemDidRequestSelectHorizontalRule(_ item: BlockInputBlockItem, blockID: BlockInputBlockID) {
        ...
        guard selectedKind == .horizontalRule || selectedKind?.isImage == true || item.isRenderedContentBlock else {
            return
        }
