...
extension BlockInputBlockItem {
    ...
    /// Scale of the style's base font relative to the system body size.
    ///
    /// `1` when no custom base font is set. Font zoom (e.g. BlockInputKitZoom) enlarges `baseText.font`, so
    /// this ratio also drives non-text block sizing (images) so the whole document scales uniformly.
    static func baseFontScale(for style: BlockInputStyle) -> CGFloat {
        let defaultBaseFont = NSFont.preferredFont(forTextStyle: .body)
        return style.baseText.font.map { $0.pointSize / max(defaultBaseFont.pointSize, 1) } ?? 1
    }
    ...
    func applyTextAttributes(for block: BlockInputBlock) {
        ...
        textStorage.removeAttribute(.blockInputLinkOpenIcon, range: fullRange)
    ...
    func updateTypingAttributesForCurrentSelection() {
        ...
        attributes.removeValue(forKey: .blockInputLinkOpenIcon)
