...
final class BlockInputDocumentScrollView: NSScrollView {
    ...
    /// While the canvas is pinch-zoomed, two-finger scroll pans the magnified content instead of scrolling the
    /// document. At 1x (not zoomed) this passes through to normal vertical scrolling.
    override func scrollWheel(with event: NSEvent) {
        if blockInputView?.routeScrollToCanvasPan(event) == true {
            return
        }
