...
extension BlockInputView {
    ...
    func refreshCompletionSession(
        ...
        originatedFromTextChange: Bool = true
    ) {
        guard isEditable,
        ...
        // A bare caret move (e.g. clicking a rendered `[[...]]` wikilink) must not START a new session; otherwise clicking
        // a link pops the note selector. Only typing opens a session; selection changes merely refresh/dismiss an open one.
        guard originatedFromTextChange || completionSession != nil else {
            return
        }
    ...
    private func completionToken(
        ...
        // Registered multi-char token triggers (e.g. the wikilink plugin's "[[") are searched first, then the
        // single-char `@`/`/` path below.
        if let customToken = customMarkupCompletionToken(in: nsText, caretOffset: caretOffset) {
            guard !completionRangeIntersectsExcludedInlineRanges(customToken.replacementRange, in: text) else {
                return nil
            }
            return customToken
        }
    ...
    private func requestCompletionSuggestions(for session: BlockInputCompletionSession) {
        ...
        let capturedSourceText = session.sourceText
        completionRequestTask = Task.detached(
            ...
            await MainActor.run { [weak self] in
                ...
                guard let currentBlock = block(withID: current.blockID),
                      capturedSourceText == currentBlock.text,
    ...
    private func presentCompletionPopup(
        ...
        presentCompletionPopup(
            popup,
            captureView: completionPopupEventCaptureView,
            layout: completionPopupLayout(for: session, state: state),
            state: state,
